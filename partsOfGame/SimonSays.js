...
let hardModeGame = {
    ...
    highlightButton: function(color) {
        ...
        let xShift = width / 8; // Same shift as in `drawButtons`
    ...
    checkPlayerSequence: function() {
      if (this.sequence.join() === this.playerSequence.join()) {
        this.nextColor(); // Proceed to the next color if correct
      } else {
        this.sequence = []; // Reset sequence if wrong
        this.level = 0;
        this.nextColor();
      }
    },
    ...
    mousePressed: function() {
      ...
      if (color) {
        ...
        // If the clicked color is incorrect, reset the game
