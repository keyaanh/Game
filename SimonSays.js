...
let currentPage = "gameSelection"; // Initial page to show game options
let sequence = [];
let userSequence = [];
let level = 0;
let gameSpeed = 1000; // Speed of the sequence for easy mode
let gameStarted = false;
let userTurn = false;
let buttonColors = ['#ff0000', '#00ff00', '#0000ff', '#ffff00']; // Red, Green, Blue, Yellow
  ...
  function draw() {
    ...
    if (currentPage === "home") {
      ...
      basketballDraw();
        ...
        easyModeGame.draw();  // Display the Easy Mode Game
    } else if (currentPage === "hardModeGame") {
        hardModeGame.draw();  // Display the Hard Mode Game
    } else if (currentPage === "levels") {
        drawSimonLevelsPage(); // New levels page
...
function drawSimonLevelsPage() {
    ...
    // Easy Mode Button
    if (button("Easy Mode", width / 2, height / 2 - 60, 250, 50) && mouseReleasedFlag) {
    ...
    // Hard Mode Button
    if (button("Hard Mode", width / 2, height / 2, 250, 50) && mouseReleasedFlag) {
        currentPage = "hardModeGame"; 
        hardModeGame.setup();         
        mouseReleasedFlag = false;    
    }
    
    if (button("Back to Game Page", width / 2, height / 2 + 60, 250, 50) && mouseReleasedFlag) {
        currentPage = "gameSelection";          
        mouseReleasedFlag = false;  
    }
}
...
let easyModeGame = {
    ...
    draw: function() {
        ...
        textSize(35);
        textAlign(CENTER, CENTER);
        text(`Level: ${this.level}`, width / 2, height - 20);

        if (this.compTurn) {
            this.showComputerSequence();
        }
    ...
    getButtonPosition: function(color) {
        switch (color) {
            case 'red': return [0, 0];
            case 'green': return [width / 2, 0];
            case 'blue': return [0, height / 2];
            case 'yellow': return [width / 2, height / 2];
        }
    },
    ...
    showComputerSequence: function() {
        if (millis() - this.lastPlayTime > this.blinkDuration) {
...
let hardModeGame = {
    ...
    draw: function() {
      ...
      textSize(35);
      textAlign(CENTER, CENTER);
      text(`Level: ${this.simonLevel}`, width / 2, height - 40);
    ...
    drawButtons: function() {
      ...
      fill(this.isButtonHighlighted && this.sequence[this.index - 1] === 'red' ? 'pink' : 'darkred');
      rect(0, 0, thirdWidth, halfHeight);
  
      fill(this.isButtonHighlighted && this.sequence[this.index - 1] === 'green' ? 'lightgreen' : 'darkgreen');
      rect(thirdWidth, 0, thirdWidth, halfHeight);
  
 
      fill(this.isButtonHighlighted && this.sequence[this.index - 1] === 'blue' ? 'lightblue' : 'darkblue');
      rect(2 * thirdWidth, 0, thirdWidth, halfHeight);
  
      // Yellow Button
      fill(this.isButtonHighlighted && this.sequence[this.index - 1] === 'yellow' ? 'lightyellow' : 'goldenrod');
      rect(0, halfHeight, thirdWidth, halfHeight);
  
      // Purple Button
      fill(this.isButtonHighlighted && this.sequence[this.index - 1] === 'purple' ? 'violet' : 'darkviolet');
      rect(thirdWidth, halfHeight, thirdWidth, halfHeight);
  
      // Orange Button
      fill(this.isButtonHighlighted && this.sequence[this.index - 1] === 'orange' ? 'lightsalmon' : 'darkorange');
      rect(2 * thirdWidth, halfHeight, thirdWidth, halfHeight);
    },
    ...
    getColorClicked: function(x, y) {
      ...
      if (x < thirdWidth && y < halfHeight) return 'red';
      if (x >= thirdWidth && x < 2 * thirdWidth && y < halfHeight) return 'green';
      if (x >= 2 * thirdWidth && y < halfHeight) return 'blue';
      if (x < thirdWidth && y >= halfHeight) return 'yellow';
      if (x >= thirdWidth && x < 2 * thirdWidth && y >= halfHeight) return 'purple';
      if (x >= 2 * thirdWidth && y >= halfHeight) return 'orange';
    }
  };